...
def _patch_retrieval_dependencies(
    ...
    *,
    ...
    monkeypatch.setattr(
        ...
        lambda: (
            sparse_provider
            or StaticSparseEmbeddingProvider(
...
def test_retrieval_search_command_falls_back_on_rerank_configuration_error(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    session = _make_session()
    workspace = _create_workspace(session)
    ...
    def raise_configuration_error() -> RecordingRerankProvider:
    ...
    assert result.exit_code == 0
    data = json.loads(result.stdout)
    assert [item["chunk_id"] for item in data["results"]] == [str(near.id)]
...
def test_retrieval_search_command_uses_bm25_strategy(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    session = _make_session()
    workspace = _create_workspace(session)
...
def test_retrieval_search_command_uses_sparse_strategy(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    session = _make_session()
    workspace = _create_workspace(session)
    ...
    assert sparse_provider.query_inputs == ["SKU-42 installation"]
    data = json.loads(result.stdout)
    assert [item["chunk_id"] for item in data["results"]] == [str(target.id)]
