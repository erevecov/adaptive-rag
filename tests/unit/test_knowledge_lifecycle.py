...
def test_resync_enqueues_ingest_job() -> None:
    session = _session()
    workspace = authoring.create_workspace(session, name="Life")
...
def test_ingest_marks_sync_and_dedup_detects_shared_hash() -> None:
    session = _session()
    workspace = authoring.create_workspace(session, name="Dedup")
    ...
    status = knowledge_lifecycle.get_source_sync_status(
        session, workspace_id=workspace.id, source_id=s1.id
    )
    assert status.status == "synced"
    assert status.latest_content_hash
    assert status.last_synced_at is not None

    report = knowledge_lifecycle.build_dedup_report(session, workspace_id=workspace.id)
