"""Tests for opt-in sparse retrieval."""
...
from adaptive_rag.retrieval.sparse import SparseRetrievalError, SparseRetriever
...
def test_sparse_retriever_scores_and_preserves_original_citation() -> None:
    ...
    _general_source, _general_document, _general_version, general = (
        _create_sparse_chunk(
            ...
            snippet="General sparse notes",
            vector=SparseEmbeddingVector(indices=(1,), values=(0.5,)),
        )
    )
    ...
    assert results[0].score == pytest.approx(1.75)
    assert results[1].score == pytest.approx(0.5)
    assert results[0].sparse_metadata == {
        "sparse_index_fingerprint": "sparse-fp:sku",
        "sparse_rank": 1,
        "sparse_score": pytest.approx(1.75),
        "used_sparse": True,
    }
...
def test_sparse_retriever_filters_before_scoring() -> None:
    ...
    wanted_source, wanted_document, _version, wanted = _create_sparse_chunk(
        ...
        vector=SparseEmbeddingVector(indices=(7,), values=(0.5,)),
