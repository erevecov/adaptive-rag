...
function RuntimePanel({
  ...
  return (
    ...
    >
      ...
      <PanelBody className="grid gap-4 p-4 pt-0 max-[680px]:gap-2 max-[680px]:p-2.5 max-[680px]:pt-0">{children}</PanelBody>
    </Panel>
  )
}
    ...
    >
      <section aria-label="Provider Connections" className="grid gap-3">
        {state === 'loading' && connections.length === 0 ? (
          <EmptyState
            aria-busy="true"
            className="border-border/60 bg-muted/20 p-4 text-left tracking-tight motion-safe:animate-pulse max-[680px]:p-2.5"
          ...
          <EmptyState className="p-4 text-left tracking-tight max-[680px]:p-2.5" data-slot-state="empty" role="status">
            No Connections Yet.
          </EmptyState>
        ) : (
          <DataList>
...
export function RuntimeSlotList({
  ...
  if (slots.length === 0) {
    return (
      <EmptyState
        className="p-4 text-left tracking-tight max-[680px]:p-2.5"
        data-slot-state="empty"
        role="status"
      >
