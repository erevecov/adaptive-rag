...
function IngestionStatus({ state }: { state: RequestState }) {
  return (
    <StatusBadge
      aria-live="polite"
      className="max-[680px]:rounded-sm max-w-full min-w-[4.75rem] max-[680px]:min-w-[4.25rem] justify-center break-all text-left max-[680px]:px-0.5 max-[680px]:text-[0.5625rem] max-[680px]:leading-snug max-[680px]:tracking-tighter"
      role="status"
      tone={requestStateTone(state)}
    >
    ...
    >
      <form className="grid gap-4 tracking-tight max-[680px]:gap-0.5 max-[680px]:p-0.5" onSubmit={onCreateProject}>
        ...
        <div className="flex flex-wrap items-center gap-2 max-[680px]:gap-0.5">
          <Button className="min-h-9 max-[680px]:min-h-4 max-[680px]:px-0.5 max-[680px]:text-[0.5625rem] max-[680px]:tracking-tighter max-[680px]:leading-snug" disabled={isBusy} type="submit">
            <ButtonLabel
              busy={isBusy}
              busyLabel="Creating…"
