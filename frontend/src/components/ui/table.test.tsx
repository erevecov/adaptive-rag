...
describe('Table density', () => {
  test('sticky header uses card tint; rows hover for scan', () => {
    ...
    expect(scroll?.className).toContain('rounded-[2px]')
    expect(scroll?.className).toContain('max-[680px]:overscroll-y-contain')
    expect(scroll?.className).toContain('max-[680px]:max-h-[min(50vh,24rem)]')
    expect(scroll?.className).not.toContain('max-[680px]:max-h-[min(50vh,0.0625rem)]')
    ...
    expect(screen.getByText('12ms').closest('[data-slot="table-row"]')?.className).toContain(
      'max-[680px]:active:bg-primary/95',
    )
    expect(screen.getByText('12ms').closest('[data-slot="table-row"]')?.className).toContain(
