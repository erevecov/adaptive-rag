...
describe('Panel density', () => {
  test('defaults to 4/8 spacing rhythm', () => {
    ...
    expect(screen.getByText('Header').className).toContain('max-[680px]:p-2.5')
    expect(screen.getByText('Body').className).toMatch(/\bp-4\b/)
    expect(screen.getByText('Body').className).toContain('max-[680px]:p-2.5')
  ...
  test('panel shell transitions colors; description stays compact', () => {
    ...
    expect(screen.getByText('Appearance').className).toContain('max-[680px]:text-sm')
...
describe('DataListItem', () => {
  test('allows truncation inside flex rows via min-w-0', () => {
    ...
    expect(row.className).toContain('max-[680px]:p-1.5')
