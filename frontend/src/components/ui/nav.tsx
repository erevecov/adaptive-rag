...
export const NavSection = forwardRef<HTMLElement, NavSectionProps>(
  ({ children, className, title, ...props }, ref) => (
    ...
    >
      {title ? (
        <h2
          className="px-2 text-[0.6875rem] font-semibold uppercase tracking-wide text-muted-foreground max-[680px]:px-1 max-[680px]:text-[0.625rem] max-[680px]:tracking-wider"
...
export const SidebarItem = forwardRef<HTMLButtonElement, SidebarItemProps>(
  ({ active = false, className, type = 'button', ...props }, ref) => (
    <button
      className={cn(
        [
          'inline-flex h-9 w-full items-center justify-start gap-2 rounded-md px-3 text-sm font-medium tracking-tight max-[680px]:min-h-11 max-[680px]:gap-1.5 max-[680px]:px-2.5 max-[680px]:text-[0.8125rem]',
