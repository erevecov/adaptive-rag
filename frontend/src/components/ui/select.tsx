  ...
  return (
    ...
    >
      <SelectPrimitive.Trigger
        ...
        className={cn(
          [
            'flex h-9 w-full items-center justify-between gap-2 rounded-md border border-input bg-background px-3 py-2 text-sm tracking-tight text-foreground max-[680px]:min-h-11 max-[680px]:gap-1 max-[680px]:px-2 max-[680px]:text-base max-[680px]:leading-snug',
      ...
      <SelectPrimitive.Portal>
        <SelectPrimitive.Content
          className={cn(
            [
              'z-50 max-h-64 min-w-[var(--radix-select-trigger-width)] overflow-hidden rounded-md border border-border bg-popover p-1 text-sm tracking-tight text-popover-foreground shadow-[var(--shadow-popover)] motion-safe:transition-colors max-[680px]:p-1',
          ...
          <SelectPrimitive.Viewport className="grid gap-1 p-0 max-[680px]:gap-1">
            {options.map((option) => (
              <SelectPrimitive.Item
                className={cn(
                  [
                    'relative flex min-h-8 max-[680px]:min-h-11 cursor-default select-none items-center rounded-sm px-3 py-1.5 text-sm tracking-tight outline-none max-[680px]:px-2 max-[680px]:text-[0.6875rem]',
