...
export const EmptyState = forwardRef<HTMLDivElement, EmptyStateProps>(
  ({ className, role = 'status', ...props }, ref) => (
    <div
      className={cn(
        'flex flex-col gap-1.5 rounded-md border border-dashed border-border/80 max-[680px]:border-border/70 bg-muted/20 p-4 text-center text-sm tracking-tight text-muted-foreground max-[680px]:gap-0.5 max-[680px]:rounded-sm max-[680px]:p-2 max-[680px]:text-xs max-[680px]:leading-snug',
...
const calloutVariants = cva(
  'rounded-md border p-4 text-sm leading-relaxed tracking-tight motion-safe:transition-colors max-[680px]:p-2 max-[680px]:text-xs max-[680px]:leading-snug',
  {
    defaultVariants: {
      tone: 'neutral',
    },
