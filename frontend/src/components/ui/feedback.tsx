...
export const EmptyState = forwardRef<HTMLDivElement, EmptyStateProps>(
  ({ className, role = 'status', ...props }, ref) => (
    <div
      className={cn(
        'flex flex-col gap-1.5 rounded-md border border-dashed border-border/80 bg-muted/20 p-4 text-center text-sm tracking-tight text-muted-foreground max-[680px]:gap-1 max-[680px]:p-3 max-[680px]:text-xs max-[680px]:leading-snug',
...
const inlineFeedbackVariants = cva(
  'text-sm font-medium leading-relaxed tracking-tight motion-safe:transition-colors max-[680px]:text-xs',
  {
    defaultVariants: {
      tone: 'neutral',
    },
