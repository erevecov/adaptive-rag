"""Routes for chat-sourced workspace knowledge proposal workflows."""
...
router = APIRouter(
    prefix="/workspaces/{workspace_id}/knowledge-proposals",
...
def list_knowledge_proposals(
    ...
    try:
        proposals = KnowledgeProposalRepository(session).list_by_workspace(
            workspace_id=workspace_id,
            status=status,
        )
    except ValueError as exc:
...
def approve_knowledge_proposal(
    ...
    try:
        if body.refined_text is not None:
            ...
            existing = repo.get(workspace_id=workspace_id, proposal_id=proposal_id)
            if existing is None:
