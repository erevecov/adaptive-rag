"""Operaciones publicas de authoring para workspaces y sources."""
    ...
    *,
    ...
    if embedding_mode not in {"dense", "dense_sparse"}:
        raise AuthoringError(
            "workspace embedding_mode must be dense or dense_sparse",
            status_code=422,
        )
...
def list_workspaces(
    ...
    *,
    ...
    return WorkspaceRepository(session).list(member_user_id=member_user_id)


def get_workspace(session: Session, workspace_id: UUID) -> Workspace:
    workspace = WorkspaceRepository(session).get(workspace_id)
    if workspace is None:
        raise AuthoringError("workspace not found", status_code=404)
    return workspace
...
def list_sources(
    ...
    *,
    ...
    get_workspace(session, workspace_id)
    return SourceRepository(session).list(workspace_id=workspace_id, filters=filters)


def get_source(session: Session, *, workspace_id: UUID, source_id: UUID) -> Source:
    get_workspace(session, workspace_id)
...
def update_workspace(
    ...
    *,
    ...
    if name is not None and not name.strip():
        raise AuthoringError("workspace name must not be empty", status_code=422)
    workspace = WorkspaceRepository(session).update(
        workspace_id,
...
def soft_delete_workspace(session: Session, workspace_id: UUID) -> Workspace:
    workspace = WorkspaceRepository(session).soft_delete(workspace_id)
    if workspace is None:
        raise AuthoringError("workspace not found", status_code=404)
    return workspace
...
def update_source(
    ...
    *,
    ...
    get_source(session, workspace_id=workspace_id, source_id=source_id)
...
def workspace_payload(workspace: Workspace) -> dict[str, Any]:
    return {
        ...
        "created_at": _datetime_payload(workspace.created_at),
        "updated_at": _datetime_payload(workspace.updated_at),
    }
