...
from adaptive_rag.db.models import Job, Source, Workspace
...
DEFAULT_WORKSPACE_NAME = "Adaptive RAG First Run"
...
Adaptive RAG first run creates a local workspace, ingests Markdown, indexes
...
def first_run_report_payload(report: FirstRunReport) -> dict[str, object]:
    return {
        ...
        "workspace": authoring.workspace_payload(report.workspace),
        ...
        "next_commands": [
            (
                ...
                f"--workspace-id {report.workspace.id} --message "
                f"{_shell_quote(report.question)}"
            ),
            f"adaptive-rag sources list --workspace-id {report.workspace.id}",
            f"adaptive-rag jobs list --workspace-id {report.workspace.id}",
        ],
    }
